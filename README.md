...
- WSL2 + Docker Engine (Ubuntu推奨)
- Java 17以上
- Maven 3.6以上
- Node.js 16以上
- npm または yarn

### 0. WSL2 + Docker Engine のセットアップ（初回のみ）

> Docker Desktop を使わず、無料で利用できる WSL2 + Docker Engine を使用します。

#### 0-1. WSL2 の有効化（PowerShell を管理者権限で実行）

```powershell
# WSL2 をインストール（Ubuntu が同時にインストールされます）
wsl --install

# 既に WSL がインストール済みの場合、バージョンを確認
wsl --list --verbose
```

> インストール後に **再起動** が求められる場合があります。再起動後、Ubuntu の初回起動が自動で始まります。
> ユーザー名とパスワードを設定してください（このパスワードは `sudo` コマンドで使用します）。

#### 0-2. Docker Engine のインストール（WSL2 Ubuntu ターミナルで実行）

プロジェクトに含まれているセットアップスクリプトを使用します：

```bash
# プロジェクトディレクトリに移動
cd /mnt/c/Users/<ユーザー名>/myWorkspace/db-performance-tuning

# スクリプトを実行
bash setup-docker-wsl.sh
```

スクリプト完了後、WSL2 を再起動して設定を反映させます（PowerShell で実行）：

```powershell
wsl --shutdown
```

#### 0-3. WSL2 再起動後の確認（WSL2 Ubuntu ターミナルで実行）

Ubuntu を再度開き、Docker が正常に起動していることを確認します：
...
`Hello from Docker!` と `active` が表示されれば成功です。

> **WSL2 のアイドルシャットダウンについて**
> WSL2 はアクティブなセッションがないとシャットダウンされる場合があります。
> `C:\Users\<ユーザー名>\.wslconfig` に以下を設定するとシャットダウンを抑制できます：
>
> ```ini
> [wsl2]
> vmIdleTimeout=0
> ```
>
> また、`docker-compose.yml` に `restart: unless-stopped` が設定されているため、
> WSL2 が再起動した際も Docker コンテナは自動的に復帰します。
...
- WSL2 + Docker Engine がインストールされているか確認してください
- WSL2 の Ubuntu ターミナル内でコマンドを実行してください（`wsl` で起動）
- Docker サービスが起動しているか確認: `sudo service docker start`
